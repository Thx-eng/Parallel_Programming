...
// 定义阈值，低于此值使用串行
#define PARALLEL_THRESHOLD 680000
...
struct ThreadData {
    ...
    int end_idx;
    segment* seg_ptr;
    string prefix;  // 对于多segment的情况，存储前缀
    vector<string>* local_guesses;  // 每个线程的本地结果
    pthread_mutex_t* mutex;  // 用于保护共享资源
};
...
public:
    ...
    vector<vector<string>> thread_local_guesses;  // 每个线程的本地猜测存储
    
    // 添加并行化的Generate函数
    void GenerateParallel(PT pt);
    
    // 线程函数声明
    static void* GenerateWorker(void* arg);
