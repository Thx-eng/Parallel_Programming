 ...
 * @param padded_buffer 用于填充消息的可重用缓冲区 (会被修改)。
 */
...
inline void PadAndInterleave_NEON(const string& input, size_t lane_index, Byte* interleaved_buffer, size_t max_n_blocks) {
    ...
    uint8_t length_bytes[8]; // 存储小端格式的 64 位长度
    for (int i = 0; i < 8; ++i) {
    ...
    for (size_t block_idx = 0; block_idx < max_n_blocks; ++block_idx) {
        ...
        if (block_idx < current_n_blocks) {
            ...
            for (int word_idx = 0; word_idx < 16; ++word_idx) {
                ...
                Byte source_bytes[4] = {0}; // 准备要写入的4字节 (小端)
                ...
                for(int k=0; k<4; ++k) {
                    ...
                    if (byte_pos < length) {
                        ...
                        source_bytes[k] = 0x00; // 填充的零字节
                         ...
                         source_bytes[k] = length_bytes[byte_pos - (paddedLength - 8)]; // 来自长度域
                    } else {
                        // 超出部分理论上不应访问，如果访问到说明 max_n_blocks 计算或逻辑有误
                        source_bytes[k] = 0xAA; // 使用特殊值标记错误，而不是0
                    }
                }
                memcpy(dest_ptr, source_bytes, 4); // 将准备好的4字节写入目标 (已经是小端)
            }
        } else {
            // 如果当前块超出实际需要，用 0 填充该通道对应的所有字
            // (也可以考虑只填充一次，然后在 NEON 加载时处理，但显式填充更清晰)
            for (int word_idx = 0; word_idx < 16; ++word_idx) {
                Byte* dest_ptr = block_base_interleaved + word_idx * NEON_LANES * 4 + lane_index * 4;
...
)
{
    ...
    // 1. 确定批次中所有密码填充后的最大块数 (仅计算 count 个有效密码)
    ...
    if (interleaved_buffer.size() < required_size) {
        // 可以选择 reserve 或 resize。Resize 更直接，因为它会实际分配并可能初始化。
        // Reserve 只保证容量，如果之后直接写入可能访问未初始化区域（虽然 PadAndInterleave 会覆盖）。
        // resize 更安全一点。如果性能关键，且确定 PadAndInterleave 会覆盖所有需要的部分，可用 reserve。
        interleaved_buffer.resize(required_size); // 调整大小
    }
    // 注意：不清零缓冲区，因为 PadAndInterleave_NEON 会覆盖所有需要的数据，包括无效通道的填充。

    // 3. 并行填充并将数据直接写入交错缓冲区
    // 注意：循环仍然是 NEON_LANES，以填充无效通道
    ...
    // NEON 通常在小端模式下工作，MD5 标准输出是小端字节序的 state[0..3]。
    // 然而，很多时候显示 MD5 哈希值时，习惯按大端（网络字节序）显示每个 32 位整数。
    // 如果你的目标是生成与其他标准 MD5 工具（如 md5sum）一致的十六进制字符串，
    // 你需要在存储 *之前* 或格式化 *之后* 进行字节翻转。
    // vrev32q_u8 可以高效地在 NEON 寄存器内完成字节翻转。
